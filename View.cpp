...
{
    auto width = VtuberDelegate::GetInstance()->getBufferWidth(id);
    auto height = VtuberDelegate::GetInstance()->getBufferHeight(id);
    ...
    _viewData[id]._deviceToScreen->ScaleRelative(screenW / width,
             -screenW / width);
    _viewData[id]._deviceToScreen->TranslateRelative(
...
void View::UpdataViewData(int id) {
    ...
    auto width = VtuberDelegate::GetInstance()->getBufferWidth(id);
    auto height = VtuberDelegate::GetInstance()->getBufferHeight(id);
    auto x = VtuberDelegate::GetInstance()->GetX(id);
    auto y = VtuberDelegate::GetInstance()->GetY(id);
    auto scale = VtuberDelegate::GetInstance()->getScale(id);
    auto _x = -static_cast<float>(RenderTargetWidth - width) /static_cast<float>(RenderTargetWidth);
    auto _y = -static_cast<float>(RenderTargetHeight-height) /static_cast<float>(RenderTargetHeight);
    if (_viewData[id]._viewMatrix) {
      _viewData[id]._viewMatrix->Scale(
        static_cast<float>(0.615f * scale),
        static_cast<float>(scale * 1.1f));
      _viewData[id]._viewMatrix->Translate(
        static_cast<float>(x + _x),
        static_cast<float>(y +_y));
    } else {
      Initialize(id);
      _viewData[id]._viewMatrix->Scale(
        static_cast<float>(0.615f * scale),
...
void View::RenderCat(int id) {
  ...
  if (isUseLive2d && _viewData[id]._mode[_mod]._haseModel) {
    Live2DManager->OnUpdate(static_cast<uint16_t>(_viewData[id]._mode[_mod]._modelId));
  }
}
...
bool View::RenderLeftHands(int id) {
  ...
  if (_viewData[id]._mode[_mod]._leftHandsCount > 0) {  
    ...
    if (!isUseLive2d) { 
      Live2DManager->OnUpdate(static_cast<uint16_t>(_viewData[id]._mode[_mod]._leftHandModelId));
    }
    isUp = false;   
  } else if (isUseLive2d)
    isUp = false;
...
bool View::RenderRightHands(int id) {
  ...
  if (_viewData[id]._mode[_mod]._rightHandsCount > 0) {
    ...
    if (!isUseLive2d) { 
      Live2DManager->OnUpdate(static_cast<uint16_t>(_viewData[id]._mode[_mod]._rightHandModelId));
    }
    isUp = false; 
  } else if (isUseLive2d)
    isUp = false;
...
void View::InitializeSpirite(int id) {
  ...
  //auto Live2DManager = Live2DManager::GetInstance();
  ...
  for (int _modelCount = 0; _modelCount < _infoReader->ModeCount; _modelCount++) {
    ...
    auto& count = _viewData[id]._mode[_modelCount]._keysCount;
    count = static_cast<uint16_t>(_modeinfo[_modelCount].KeysCount);
    ...
    if (strcmp(_modeinfo[_modelCount].LeftHandImagePath, "") != 0) {
      ...
      //load left hands
      auto& count = _viewData[id]._mode[_modelCount]._leftHandsCount;
      count = static_cast<uint16_t>(_modeinfo[_modelCount].ModelLeftHandCount);
      for (int i = 0; i < count; i++) {
    ...
    if (strcmp(_modeinfo[_modelCount].RightHandImagePath ,"")!=0) {
      ...
      //load right hands
      auto& count = _viewData[id]._mode[_modelCount]._rightHandsCount;
      count = static_cast<uint16_t>(_modeinfo[_modelCount].ModelRightHandCount);
      for (int i = 0; i < count; i++) {
