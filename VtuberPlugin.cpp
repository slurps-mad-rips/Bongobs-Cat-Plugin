...
const char * VtuberPlugin::VtuberPlugin::VtuberGetName(void*) { return "bongobs cat"; }

void * VtuberPlugin::VtuberPlugin::VtuberCreate(obs_data_t *settings,
                 obs_source_t *source)
{
  auto vtb = new Vtuber_data { .source = source, .modelId = 0 };
...
void VtuberPlugin::VtuberPlugin::VtuberDestroy(void *data) {
  ...
  std::unique_ptr<Vtuber_data>(static_cast<Vtuber_data*>(data));
}

void VtuberPlugin::VtuberPlugin::VtuberRender(void *data, gs_effect_t*) {
...
void VtuberPlugin::VtuberPlugin::Vtuber_update(void *data, obs_data_t *settings) {
  ...
  auto vtb = static_cast<Vtuber_data*>(data);
